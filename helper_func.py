...
def create_neg_keys(P):
    B, D = P.shape
    mask = ~torch.eye(B, dtype=torch.bool, device=P.device)
    N = P.unsqueeze(0).expand(B,-1,-1)[mask].view(B, B-1, D)
    return N
...
def create_neg_keys_2(A, P):
    ...
    M = A.unsqueeze(0).expand(B,-1,-1)[mask].view(B, B-1, D)
    N = P.unsqueeze(0).expand(B,-1,-1)[mask].view(B, B-1, D)
    return N, M
...
def create_neg_keys_3(A, P, NN):
    ...
    # Only append the tailored hard negative (not other people's) to maintain logical symmetry
    O = NN.unsqueeze(1) # shape [B, 1, D]
    
    neg_forward = torch.cat((N, O), dim=1) # query checks against other Satellites + its own hard Satellite
    neg_reverse = M # key checks against other Grounds
    return neg_forward, neg_reverse